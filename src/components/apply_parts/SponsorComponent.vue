<template>
    <div>
        <h2 class="text-2xl border-b-2">Sponsor</h2>
        <br /> 
        <form class="w-full lg:w-1/2" @submit.prevent="$emit('save')"> 
            <label class="my-2" v-for="(value, index) in sponsor" :key="index">
                <span v-if="index == '_id' "></span>
                <span v-else-if="index == 'phone_no_one' ||  index == 'phone_no_two' ">
                    <span class="block font-medium text-slate-700 text-sm dark:text-white capitalize">{{ form_text_parser(index) }}</span>
                    <input type="number" class="input-type-1 bg-transparent" v-model="sponsor[index]" />
                </span>
                <span v-else>
                    <span class="block font-medium text-slate-700 text-sm dark:text-white capitalize">{{ form_text_parser(index) }}</span>
                    <input type="text" class="input-type-1 bg-transparent" v-model="sponsor[index]" />
                </span>
            </label>
...
export default {
    computed: {
        sponsor() {
            return this.$store.getters['applicant/details'].info.sponsor
        }
    },
    methods: {
        form_text_parser
