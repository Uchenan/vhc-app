<template>
  <div>
    ...
    <div class="mx-auto w-min">
      <img
        class="w-full mx-auto h-20 bg-gray-200 rounded-tr-lg rounded-tl-lg"
        ref="profile_pic"
        :src="staff.bio.img_url"
      />
      <input
        type="file"
      ...
      />
      <button
        class="block w-max bg-primary text-white rounded-br-lg rounded-bl-lg py-2 px-3 hover:bg-primarydark"
        @click="clickImg"
      >
        Change Picture
      </button>
    ...
    <div class="flex flex-col items-center w-full py-2 tweak-one bg-white">
      <span class="text-2xl font-bold">{{
        `${staff.bio.surname.toUpperCase()} ${staff.bio.other_names.toUpperCase()}`
      }}</span>
      <span>ID: {{ staff.account.ref_id }}</span>
      <button
        @click="updateHandler"
        class="py-1 px-6 bg-secondary hover:bg-secondarydark hover:text-white rounded-md my-2 focus:outline-none"
      >
    ...
    <span class="block text-2xl my-3 font-bold border-b-2 flex items-center"
      >Personal</span
    >
    <form>
      <label class="my-2">
        <span class="block font-medium text-slate-700 text-sm dark:text-white"
          >Title</span
        >
    ...
    <form>
      ...
      <label class="my-2">
        ...
        <input
          type="text"
          class="input-type-1"
          v-model="staff.account.security_question"
        />
      </label>
    ...
    <form>
      ...
      <label class="my-2">
        ...
        <div class="flex items-stretch mb-3">
          <button
            class="bg-primary dark:bg-black text-white w-min px-4 rounded-tl-lg rounded-bl-lg focus:outline-none"
            disabled
          >
            +234
          </button>
          <input
            type="number"
            v-model="staff.contact.tel_one"
            class="w-full p-2 border dark:bg-gray-700 text-gray-500 dark:text-white rounded-tr-lg rounded-br-lg border-slate-300 dark:border-slate-700 focus:outline-none"
          />
        </div>
      </label>
    ...
    <button
      @click="updateHandler"
      class="py-2 px-8 bg-secondary hover:bg-secondarydark hover:text-white rounded-sm my-2 mb-6 focus:outline-none"
    >
      <i class="mdi mdi-update"></i> Update
    </button>
  </div>
...
export default {
  ...
  computed: {
    staff() {
      let res = Object.assign(
        {},
        this.$store.getters["staff/specific"](this.$route.params.ref_id)
      );
  ...
  methods: {
    updateHandler() {
      this.$store.dispatch("staff/update", this.staff).then((doc) => {
        if (doc === true) {
          openModal(ModalComponent, {
            modal_type: "success",
            modal_body: `${this.staff.account.ref_id} details were updated successfully`,
          });
        } else {
          openModal(ModalComponent, {
            modal_type: "failed",
            modal_body: "Account updated failed due to internal errors",
          });
        }
      });
    },
    ...
    selectImg() {
      ...
      if (img) {
        ...
        reader.onloadend = async () => {
          ...
          const modal = await openModal(UploadFileComponent, {
            file_preview: this.selectedImg,
          ...
          modal.on("return", (value) => {
            if (value === true) {
              ...
              imageStaffUploader(this.staff.account.ref_id, this.selectedImg)
                .then((res) => {
