    ...
    <div>
        <div class="bg-primary dark:bg-primarydark px-4 py-2 text-white mb-3 flex items-center">
            <div class="flex flex-col">
                ...
                <span>{{ $store.getters['user/fullNameStudent'] }}</span>
            </div>
            <div>
                <router-link
                    ...
                    :to="{name: 'suProfile', params: { studentId: $store.getters['user/details'].account.ref_id }}">
...
export default {
    ...
    data() {
        return {
            navLinks: [
                {name: "Certificates", path: "test", icon: "file"},
                {name: "Bills", path: "test", icon: "cash"},
